...
import { ISessionRepository, TSessionDbRow } from "../types/session.types.js";

export class SessionRepository implements ISessionRepository {
  async createSession(userId: string, token: string, ipAddress: string, userAgent: string, deviceInfo: object, expiresAt: Date): Promise<void> {
    await db.query(
  ...
  async findValidSession(userId: string, token: string): Promise<TSessionDbRow | null> {
    const query = `
    ...
    return result.rows[0] as TSessionDbRow | null;
  }

  async updateLastActivity(userId: string, token: string): Promise<void> {
    await db.query(
