  ...
  const renderPageNumbers = () => {
    ...
    if (totalPages <= maxVisiblePages) {
      for (let i = 1; i <= totalPages; i++) {
        pages.push(
          <PaginationPage key={i} href={`?page=${i}&${searchParams.toString()}`} current={i === page}>
            {i}
          </PaginationPage>
        );
      }
      ...
      pages.push(
        <PaginationPage key={1} href={`?page=1&${searchParams.toString()}`} current={page === 1}>
          1
        </PaginationPage>
      );
      ...
      pages.push(
        <PaginationPage key={totalPages} href={`?page=${totalPages}&${searchParams.toString()}`} current={page === totalPages}>
          {totalPages}
        </PaginationPage>
      );
    }
            ...
            <>
              ...
              {pagination && pagination.totalPages > 1 && (
                <div className="mt-20 flex justify-start lg:mt-24">
                  <Pagination>
                    <PaginationPrevious
                      href={pagination.page > 1 ? `?page=${pagination.page - 1}&${searchParams.toString()}` : null}
                    />
                    <PaginationList>{renderPageNumbers()}</PaginationList>
                    <PaginationNext
                      href={pagination.page < pagination.totalPages ? `?page=${pagination.page + 1}&${searchParams.toString()}` : null}
                    />
                  </Pagination>
                </div>
              )}
